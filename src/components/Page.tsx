...
function WikiSpacePage(props: { page: Page }) {
  ...
  const blocksListState = useObjectState<CausalArray<Block>>(page?.blocks, {
    debounceFreq: 50,
  });
  ...
  }, [wikiWriteFlags, wikiMembers, page, launcher]);
  ...
  useEffect(() => {
    ...
    if (
      size !== undefined &&
      size > 0 &&
      blocksListState?.getDebounceFreq() !== undefined
    ) {
  ...
  const focusOnAdjacentBlock =
    ...
    (thisBlock: Block, distance = 1) => {
      const adjacent = blocksListState?.value?.lookup(
        blocksListState?.value?.indexOf(thisBlock) + distance,
      );
  ...
  const blockElements = blocksListState
    ?.getValue()
    ?.contents()
    .map((block, index) => (
      <Draggable
        isDragDisabled={!editable}
        draggableId={block.getLastHash()}
        index={index}
        key={block.getLastHash()}
      >
        {provided => (
          <div
  ...
  const onNewPicture: React.ChangeEventHandler<HTMLInputElement> = (
    ...
    <>
      ...
      >
        <Box style={{ minWidth: '100%', height: '100%' }} width="100%">
          {page === undefined && <Typography>Loading...</Typography>}
          ...
          {page !== undefined && (
            <Box>
              ...
              {!blocksListState?.getValue()?.contents().length &&
                wikiTitleState?.getValue()?.title?.getValue() !== undefined && (
                  <Stack
                    direction="row"
                    spacing={3}
                  ...
                  >
                    ...
                    <Button
                      aria-label="add something to the page"
                      variant="contained"
      ...
      >
        ...
        >
          <ListItemIcon>
            <img
              src="icons/streamlinehq-common-file-horizontal-text-files-folders-48.png"
              style={{
                width: '24px',
                height: '24px',
                margin: '1px',
        ...
        >
          <ListItemIcon>
            <img
              src="icons/streamlinehq-picture-sun-images-photography-48.png"
              style={{
                width: '24px',
                height: '24px',
                margin: '1px',
